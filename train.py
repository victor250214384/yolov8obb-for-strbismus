...
from utils.loss import ComputeLoss,KLDloss
...
def parse_opt(known=False):
    ...
    parser.add_argument('--evolve', type=int, nargs='?', default=300, help='evolve hyperparameters for x generations')
    ...
    parser.add_argument('--lion', action='store_true', help='use torch.optim.Adam() optimizer')
    ...
    parser.add_argument('--is_use_DP', action='store_true', help='DP train')
...
def train(hyp,  # path/to/hyp.yaml or hyp dictionary
    ...
    if opt.lion:
        # optimizer = Lion(g0,lr=0.001, betas=(0.8, 0.99))  # adjust beta1 to momentum
        # optimizer = Lion(g0,lr=0.001, betas=(0.8, 0.81))  # adjust beta1 to momentum
        # optimizer = Lion(g0,lr=0.001, betas=(0.6, 0.7))  # adjust beta1 to momentum
        optimizer = Lion(g0,lr=0.001, betas=(0.849, 0.85))  # adjust beta1 to momentum
    else:
    ...
    train2_loader = create_dataloader(train_path, imgsz, batch_size // WORLD_SIZE * 2, gs, names, single_cls,
                                    hyp=hyp, cache=None if noval else opt.cache, rect=True, rank=-1,
                                    workers=workers, pad=0.5,
                                    prefix=colorstr('train2: '))[0]
    ...
    for epoch in range(start_epoch, epochs):  # epoch ------------------------------------------------------------------
        ...
        for i, (imgs, targets, paths, _) in pbar:  # batch -------------------------------------------------------------
            ...
            with amp.autocast(enabled=cuda):
                ...
                #     loss, loss_items = compute_loss(pred, targets.to(device),'l1')  # loss scaled by batch_size
                # else:
                #     loss, loss_items = compute_loss(pred, targets.to(device),'l2')  # loss scaled by batch_size
        ...
        if RANK in [-1, 0]:
            ...
            results_train, maps, _ = val.run(data_dict,
                                        batch_size=batch_size // WORLD_SIZE * 2,
                                        imgsz=imgsz,
                                        model=ema.ema,
                                        single_cls=single_cls,
                                        dataloader=train2_loader,
                                        save_dir=save_dir,
                                        plots=False,
                                        callbacks=callbacks,
            ...
            # print(results_train)
            print_mutation(keys, results_train, hyp.copy(), save_dir, opt.bucket, 'evolve_train.csv')
        ...
        if RANK in [-1, 0]:
            ...
            # print(results)
            print_mutation(keys, results, hyp.copy(), save_dir, opt.bucket, 'evolve.csv')
...
def main(opt, callbacks=Callbacks()):
    ...
    evolve_yaml, evolve_csv, evolve_csv_train  = save_dir / 'hyp_evolve.yaml', save_dir / 'evolve.csv', save_dir / 'evolve_train.csv'
    if opt.bucket:
    ...
    # Plot results
    plot_evolve(evolve_csv, 'Test')
    plot_evolve(evolve_csv_train, 'Train')
