...
import cv2
from utils.rboxs_utils import poly2hbb, rbox2poly,rbox2poly_gt
...
from utils.general import (LOGGER, box_iou, check_dataset, check_img_size, check_requirements, check_yaml,
                           ...
                           scale_coords, scale_polys, xywh2xyxy, xyxy2xywh, non_max_suppression_obb,scale_coords2)
...
def run(data,
    ...
    for batch_i, (im, targets, paths, shapes) in enumerate(pbar):
        ...
        for si, pred in enumerate(out): # pred (tensor): (n, [xylsθ, conf, cls])
            ...
            #     # labels[:, 1:5]=labels[:, 1:5]*((imgsz)/ori_img.shape[1])
    ...
    if save_json and len(jdict):
        ...
        anno_json = str(Path(data.get('path', '../coco')) / 'annotations/test_datasetdemo.json')  # annotations json
